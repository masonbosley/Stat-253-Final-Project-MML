...
```{r}
# recipes & workflows
full_rec <- recipe(Salary ~ G+ GS +FG + MP + PTS +STL+Age+TRB+AST+WS+X3P+X3PA+X2P+X2PA+FT+FTA, data = nba_clean) %>%
...
knn_rec <- recipe(Salary ~ . , data = nba_clean) %>%
...
best_penalty <- select_best(nbatune_output, metric = 'rsq')
best_penalty
```
